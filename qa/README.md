...
| `SBPR.QaHarness.T022/Fixtures/` | **M3 (vanilla fixtures + crash-safe owned cleanup, engine-free):** owned-resource ledger core adopted from the reviewed prebuild (`ResourceCategory`/`OwnedResourceId`/`FixtureBounds`/`ResourceAllowlist`/`FixturePlan`/`FixturePlanValidator`/`IFixtureWorld`/`OwnedResourceLedger`/`SnapshotCodec`) — product identity/AP/ownership/signature/verdict/journal/cache are **structurally unrepresentable** (closed enum + closed allowlist). Adds the canonical `VanillaFixtureManifest` (real vanilla allowlist + bounds + `SBPR_`-prefix/denylist product-id guard), the `SeamFixtureWorld` bridge from the ledger port to the additive `IVanillaFixtureSeam` (additive-only, product-reject, drift-guard — no clone-and-strip path), and the execution-time `FixtureAuthority`/`FixtureProvisioner` recheck (server-role + world-loaded + delivering-peer + admin re-read at the moment of Ensure/Cleanup). **No craft/upgrade/transfer/tamper actions (those are later M-cards); no live TCP/ZRpc pump — helper stays inert.** | M3 → M4 |
