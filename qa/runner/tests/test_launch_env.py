...
def test_allowlist_is_exactly_the_five_non_secret_launch_vars() -> None:
    # The three arming env vars PLUS the M6-JOIN connect target PLUS the M6-JOIN3
    # server-password FILE PATH — all non-secret, all ride the same 0644 sidecar. The
    # password VALUE is NOT here (it lives in the mode-0600 file this path names); a
    # secret (HMAC/operator token) is still refused.
