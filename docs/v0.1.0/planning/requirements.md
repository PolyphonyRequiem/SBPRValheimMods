...
5. **Painted Signs** — ONE buildable sign (`piece_sbpr_sign`, 2 Wood), placed UNPAINTED; painted after placement via a **combined Paint+Text panel** (custom uGUI) that sets a **text color AND a border color** (two-tone) — one pigment per filled color slot, border optional, re-paint re-consumes; text is free via `{Update Text}` and locked until a color is chosen. Both colors persist via ZDO (`SBPR_SignTextColor`/`SBPR_SignBorderColor`). Pin path deferred/unregistered (single combined Paint+Text panel, two-tone, Daniel 2026-06-05; supersedes the 6/04 apply-ink model)
...
- **Model (LOCKED 2026-06-05, Daniel — supersedes the 6/04 apply-ink lock):** ONE buildable sign, placed UNPAINTED, painted afterward via a **custom combined Paint+Text uGUI panel** that sets a **text color AND a separate border color** (two-tone returns). The 6/04 "apply-ink-item, single color, no UI" model is retired as the entrypoint (its ZDO-write + tint logic is reused as the panel's backend). The four-variant buildable sprawl stays dropped — still ONE piece.
  - **Build:** `piece_sbpr_sign` ("Painted Sign"), placed via the Trailblazer's Spade build menu, **2 Wood**. Clone of the vanilla wood `sign` prefab; ships in its plain wood (unpainted) material. Pigment is NOT a build ingredient.
  - **Paint+Text panel:** interacting with a placed sign opens a **custom combined uGUI panel** (replaces the vanilla text dialog). PAINTING section: a **Set Text Color** swatch row + a **Border Color** swatch row (Red/Blue/Black/White active, extra slots disabled), a crafting-style **Cost** display with pigment icons (one pigment per filled slot; same color both slots = 2 of it; border optional; ≥1 color required), and a **`{Paint this and consume}`** button (disabled unless the player holds the pigments; removes them, tints board + border, writes ZDO; re-paint re-consumes). TEXT section: a text field **locked until ≥1 paint color is chosen** + a **`{Update Text}`** button (free — no pigment cost).
- **Color state:** stored per-instance on the sign's ZDO as two string fields `SBPR_SignTextColor` + `SBPR_SignBorderColor` (each `red`/`white`/`blue`/`black`, or `""` = unset). Owner-write via `ZNetView.ClaimOwnership()` + `ZDO.Set(string,string)` (mirrors the `CairnTag` tier pattern). Persists across reloads + syncs to clients; both tints re-applied on spawn via the `SignTag` component. ⚠️ The current `Signs.TintRenderers` tints **all** renderers one color — a **separate border renderer/material** must be identified on the vanilla sign mesh (or added via kitbash) to render the two tones independently (open technical question for the engineer).
- **Panel seam (clean-room):** intercept the placed sign's text-edit interaction (the vanilla `TextReceiver`/`Sign.UseItem` / `TextInput` surface) to open the custom panel instead of the vanilla single-line dialog. The existing `SignPaintPatch.cs` apply-ink prefix is **retired as the player entrypoint** but its inventory-consume + ZDO-write + tint helpers are **reused as the panel's backend**. Signatures (`Sign : Hoverable, Interactable, TextReceiver`; `bool UseItem(Humanoid, ItemDrop.ItemData)`; `Inventory.RemoveItem`/`CountItems`; `ZNetView.IsOwner/ClaimOwnership/GetZDO`; `ZDO.GetString/Set`) confirmed against `assembly_valheim.dll` **public metadata** — not decompiled IronGate source. uGUI panel built clean-room (no copied vanilla UI prefab).
- **Pin behavior (deferred, currently UNREGISTERED):** the `Minimap.AddPin` pin path stays unregistered (dead code) for v0.1.0. If later wired, the pin reflects the sign's **text** color, no-op if `nomap=ON`. Separate follow-up.
