...
>
> **§A2.6 — Paint reads TRUE: stained-wood albedo basis (Option B, LOCKED Daniel 2026-06-21, card t_6cc9f652).** The board + frame tints land on the right render layer (per-renderer MPB `_Color`, the row above), but `_Color` **multiplies** over each renderer's albedo (`_MainTex`), and the sign board + the four border bars all carry the vanilla **brown wood** albedo. So a painted surface rendered as `brown × _Color` — **muddy**: white only brightened the brown (never read white) and saturated colours, especially **red, went dark maroon** (Daniel, colour-blind, could not read the borders as tinted; the GPU harness *measured* it — border red `(135,69,67)` pre-fix vs the `(0.85,0.18,0.18)` `_Color` written, i.e. a real multiply loss, not a perception miss). This is the layer #224 (t_f3310406) did NOT address — #224 fixed *which layer wins*, this fixes *what it multiplies over*.
> - **Design choice: STAINED WOOD, not a flat painted plaque (Daniel locked Option B over Option A).** The chosen hue must read TRUE **with the wood grain still visible** — a translucent stain over a neutralised grain base, not an opaque flat plaque.
> - **Mechanism:** when tinting a renderer, push — through the SAME per-renderer MPB as `_Color` — a **neutralised copy of the wood albedo** as `_MainTex`: per-texel greyscale (kills the brown HUE) then remap each texel into a light **stain band** `[StainFloor, StainCeil]` across the texture's OWN luminance range (preserves grain CONTRAST; the band is held below pure white so the brightest grain never clamps flat — that flat-white blowout was the rejected first cut). `_Color` then multiplies over light neutral grain → true hue, grain showing. Built once per source texture, **cached** (board + 4 bars share one wood texture → built once). **Client/graphics only** (`SystemInfo.graphicsDeviceType == Null` → no-op, bare `_Color` fallback) so headless server + tests are unaffected. Lives in the same MPB as `_Color`, so the MaterialMan/`WearNTear.Highlight` wipe drops both together and the existing `SignTag` 0.3 s mesh re-assert re-pushes both — no new failure mode, #224 preserved. `RestoreRenderer` (`∅ None`) re-pushes the material's OWN albedo so a cleared surface returns to true brown wood, not the neutral grain.
> - **Empirical close (GPU harness, Niflheim, full direct sun = worst case for grain):** board white `(212,159,114)` brown → light grain; border red `(135,69,67)` maroon → **`(222,98,98)` vivid red**; grain visible on both surfaces; blue letters unaffected. Code: `Signs.TintRenderer` / `GetNeutralBaseFor` / `BuildNeutralBase` / `RestoreRenderer`. **AC (stain):** painted colour reads true (board light, red vivid not maroon); wood grain remains visible (not flat plaque); `∅ None` returns to plain brown wood; #224 winning-layer + re-assert preserved; clean build 0 warn (TWAE). Real close is Daniel's eyes on a player-placed sign in varied light (AT-SIGN-STAIN).
