...
| **Pigment tint** | The cairn's bound pigment color tints the stones by **cloning the donor stone material once and multiplying its `_Color`** (one shared tinted material for the whole pile; texture detail survives because `_Color` multiplies the albedo), re-applied on spawn from ZDO. |
