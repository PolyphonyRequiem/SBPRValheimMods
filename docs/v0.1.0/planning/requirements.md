...
- **Visual identity (LOCKED):** a Cairn is a **non-burning stone marker — no light, no heat, no fuel, no fire SFX** at any tier. It is cloned from the vanilla `bonfire` prefab as a STRUCTURAL base only (WearNTear/Piece/ZNetView); on the client the donor fire is neutralized deterministically by component type (`Fireplace` destroyed; `Light`/`ParticleSystem`/`SmokeSpawner`/`EffectArea`/`AudioSource`/`ZSFX` disabled; donor mesh renderers hidden) and a runtime stack of `rock_low` clones (T1=4 → T5=12 stones) is shown instead. Comfort comes from the `SE_Rested` patch, NOT from fire. See `Features/Cairns/CairnTag.cs`.
