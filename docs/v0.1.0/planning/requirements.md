...
| **Banner dimensions & seating (NEW 2026-06-09, card t_5756cd21)** | The cloth is scaled by **measure-and-normalize**, NOT a single scalar — the donor `default` cloth mesh is a flat Y-Z sheet (native `(X 0, Y 2.974, Z 1.1802)`, verified via vprefab on `piece_banner01/02/04/11`, all identical), so **Z carries the horizontal WIDTH and Y the vertical DROP; X is zero-thickness and is guarded/inert.** Each in-plane axis normalizes off `clothMesh.bounds.size` to a target metre size: **width(Z) ≈ 0.236 m (`TargetBannerWidthZ`), drop(Y) ≈ 0.892 m (`TargetBannerDropY`)** (FLAGGED — starting eyeball derived from Daniel's 3×-wide/2×-long report; final metres bake here on in-game sign-off). The retired uniform `BannerScale = 0.6` could not correct an *anisotropic* error. The cloth pivot sits at its top and hangs down, so the pivot is seated **`TargetBannerDropY × BannerSeatDropFrac` (= half a drop) above the pile top** (`BannerSeatDropFrac`, which replaced the retired `BannerLiftY` that buried ~0.12 m of the resized cloth underground at T1), keeping the ribbon clear of the terrain plane and the cosmetic flame (the `BannerOffsetXZ` side-nudge still applies). The shader-driven wave is preserved (X scale left at 1); wind *responsiveness* is the separate card t_6a55d059. |
