...
| **Replant Grass** | **Three grass-restore widths (1.5m / 3m / 5m)** that mirror the vanilla **Cultivator's replant ("Grass") mode** — restore/seed grass over the selected footprint, with **NO terrain raise/level/cultivate at ANY width**. The three widths mirror the path widths for a consistent three-width UX (Daniel, playtest 2026-06-05). Each width scales **only the grass/paint footprint** (`TerrainOp.m_settings.m_paintRadius`); `m_settings.m_level`/`m_smooth`/`m_raise` are left at the donor op's stock `false` so no width can flatten, smooth, or raise terrain. ⚠️ **Correction (Daniel, playtest 2026-06-04) — still in force:** this is NOT the `cultivate` (soil-tiller) op. An earlier build cloned `cultivate` at a forced 5m radius — an "UBER level" that flattened/cultivated a huge area. PR #16 fixed it to clone `replant`; this slice extends that fix to three grass-restore widths WITHOUT reintroducing terrain modification (the level/smooth radii are never written). ⚠️ **Donor-system correction (t_1d6fa833, 2026-06-09):** the grass op clones the **modern `replant_v2` (a `TerrainOp`)** — the op the vanilla Cultivator actually uses — NOT the deprecated legacy `replant` (`TerrainModifier`) it cloned before. The legacy donor was a persistent ZNetView-bearing networked piece that *coexisted with and fought* our path op (the "grass vs trail" conflict Daniel reported); `replant_v2` bakes its paint into the heightmap compiler and self-destructs, so it cleanly regrasses over a path just like the Cultivator does, and leaves no orphan ZDO. Root cause + evidence: `docs/investigations/2026-06-09-replant-grass-fights-path-terrainop-vs-terrainmodifier.md`. ⚠️ **Donor-resolution timing (t_d84d2c17, 2026-06-09):** because `replant_v2` is a `TerrainOp` with **no `ZNetView`**, it is in NEITHER of `ZNetScene`'s serialized prefab lists, so `ZNetScene.GetPrefab("replant_v2")` returns null at every ZNetScene phase on a dedicated server (the v0.2.12 regression: the boot logged it null 3× and all three replant pieces silently vanished, dropping the spade menu 12→9). The fix resolves the donor from the **vanilla Cultivator's build `PieceTable`** during the **ObjectDB-wiring phase** (`Assets.FindOpInToolPieceTable("Cultivator", "replant_v2")`), where the live reference exists; the three replant widths therefore register in the ObjectDB pass (`DoObjectDBWiring`), not at `ZNetScene.Awake`. The path widths keep registering at `ZNetScene.Awake` (their `path` TerrainModifier donor IS in ZNetScene). If the donor still can't be resolved, registration **fails loud at ERROR** rather than silently dropping the pieces. |
...
- **Class:** clone the `Hoe` item prefab, replace its `m_buildPieces` PieceTable with a Trailborne-specific table that exposes three "path" entries (1.5m / 3m / 5m widths) plus **three replant entries** at the same 1.5m / 3m / 5m widths. The paint operations bottom out at `TerrainModifier.PaintType.Path` / `Reset` — vanilla already supports the brush widths via radius parameters; we wrap them at our widths. The replant entries clone the vanilla `replant_v2` op (a `TerrainOp` — the Cultivator's grass op) and scale **only its grass/paint radius** (`m_settings.m_paintRadius`; the level/smooth/raise flags are left at stock `false` — see replant mechanic below).
- **Replant mechanic (3 widths — Daniel playtest 2026-06-05; supersedes the single-op form from 2026-06-04):** clone the vanilla **`replant_v2` prefab** (the Cultivator's "Grass" mode — prefab `replant_v2`, verified against this build's assembly_valheim.dll + prefab payload: it carries `Piece` + `TerrainOp` and no `ZNetView`) **three times** and register `piece_sbpr_replant_narrow/standard/wide` at **1.5m / 3m / 5m**, mirroring the three path widths. Each clone scales **ONLY `m_settings.m_paintRadius`** (the grass/vegetation footprint) and leaves `m_settings.m_level`/`m_smooth`/`m_raise` at the donor op's stock values, so every width behaves like the Cultivator's "Grass" mode — a grass-restore brush that regrows grass on dirt with **no terrain raise/level/cultivate at any width**. ⚠️ The original M3 build wrongly cloned the **`cultivate`** prefab (the soil-tiller that turns ground into farmland) at a forced **5m** radius, producing a wide terrain-modifying "UBER level" op (the PR #16 bug). The fix was to clone the grass-restore op instead; the t_1d6fa833 fix then corrected the donor *system* from the legacy `replant` (`TerrainModifier`) to the modern `replant_v2` (`TerrainOp`) — the op the Cultivator actually uses — fixing the grass-vs-path conflict, and this slice scales it to three grass-restore widths while **preserving that fix by construction** — because the level/smooth radii are never written, no replant width can reintroduce terrain modification. `cultivate` is the Cultivator's farming job and stays out of the spade's lane.
  - **Donor-resolution timing (t_d84d2c17, fixes the v0.2.12 dedicated-server regression):** `replant_v2` is a `TerrainOp` with **no `ZNetView`**, so it is in NEITHER of `ZNetScene`'s serialized prefab lists (`m_prefabs` / `m_nonNetViewPrefabs`) — `ZNetScene.GetPrefab("replant_v2")` is null at **every** ZNetScene phase on a dedicated server (v0.2.12 logged it null 3× at `ZNetScene.Awake`, skipping all three replant pieces → spade menu 12→9, grass tool gone). The live `replant_v2` reference exists only as a baked asset reference inside the **vanilla Cultivator's build `PieceTable`**, which is reachable once `ObjectDB` is populated. Resolution therefore happens in the **ObjectDB-wiring phase** via `Assets.FindOpInToolPieceTable("Cultivator", "replant_v2")` and the three replant variants register in `Trailblazing.RegisterReplantVariantsFromCultivator` (called from `DoObjectDBWiring`, before the spade `PieceTable` is assembled), not at `ZNetScene.Awake`. The clone is made from the resolved template (`Assets.ClonePrefabTemplate`) under the inactive holder so the `TerrainOp.Awake` (which bakes-then-self-destructs) never fires during registration. Path variants are unaffected — their `path` `TerrainModifier` donor IS in ZNetScene, so they still register at `ZNetScene.Awake`. If the donor can't be resolved, registration **fails loud at ERROR** (no silent skip — a shipped feature that silently doesn't register is itself a bug; mirrors the PatchCheck/SpecCheck philosophy).
