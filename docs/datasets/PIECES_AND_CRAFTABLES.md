...
- **Iron Compass** (Trailborne v3 Swamp — the earned no-map orientation payoff). **SPECCED — see `docs/v3/planning/iron-compass-impl-spec.md`** (card t_d35405e3, proposed; blocked on Daniel's 4 open questions). A worn, Iron-gated **Trinket** (`SBPR_IronCompass`, crafted at the Explorer's Bench) whose **HUD overlay** finally grants cardinal orientation — *without ever touching the local map*. The map keeps its Daniel-locked no-north disorientation (cartography §2H.1); the compass is the **separate earned tool**. Implementation: a `[HarmonyPatch(Hud,"Awake")]` postfix mounts an `SBPR_CompassHud` UGUI overlay under `Hud.instance.m_rootObject`; the needle is driven from `GameCamera.instance.transform.eulerAngles.y` (lerp lag = the "slight lag"), pitch mapped to a ~45° UI tilt. Gated on **equip** (Trinket slot via `GetEquippedItems()`), not mere carry. Pure HUD-overlay + item — no game-state patches, no map mutation.
