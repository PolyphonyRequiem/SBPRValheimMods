...
| Config | `SunstoneLens` section (live-tunable): `MaxCharge` (100), `DrainPerSecond` (0.33 ≈ 5 min full→empty), `ChargePerSecond` (1.0 ≈ 1.7 min empty→full), `DetectRadius` (50 m), `DetectIntervalSeconds` (0.5), `ClearWeatherNames` (optional allowlist; empty = m_isWet-driven). 🔄 **World-halo render knobs (card t_68672b6b — replace the screen-space `RingRadiusPx`/`RingCenterOffsetY`/`RingIconMinPx`/`RingIconMaxPx`):** `HaloRadiusMin` (~1.2 m), `HaloRadiusMax` (~3.0 m), `HaloScaleMax`/`HaloScaleMin` (tune), `HaloEyeOffsetY` (~0), `RingMaxIcons` (12, carried), `ShowEmptyRing` (true — faint solar ring), `ShowDepletedHint` (false), `DumpUnmappedCreatures` (**true** — NEW, startup unmapped-creature dump), `DebugTextReadout` (false), `DebugMount` (true this cut — bake false once confirmed in-game). |
| Status | IMPLEMENTED (code + spec + SpecCheck item row, card t_2fd7bc7f, 2026-06-16). **Trophy-ring render BUILT** (card t_b8a19487, 2026-06-19). **HUD-render bug FIXED** (card t_d5949685, 2026-06-19 — dead-Update-pump, `_content`-child toggle). 🔄 **RENDER REDESIGN → WORLD-SPACE EIDETIC HALO** (DESIGN card t_68672b6b, Daniel 2026-06-21 "make the standalone ring eidetic"; 4 knobs LOCKED — head-halo placement, variable radius+scale, hybrid trophy-less + startup dump, flat billboarded sprites; doc-review WAIVED "just implement as directed"). Design doc updated; **engineer-ui impl card carries the build.** SpecCheck manifest unchanged (render-only). **logs-green ≠ playable** — the world halo is a visual feature; AT-EIDETIC-1..5/AGGRO/CAMREL/OCCLUDE/PERF/MINIMAP-UNAFFECTED close only on Daniel's in-game GPU-client check. |
