...
| Visual notes | **Held mesh = procedural blank-leather field-map sheet (ADR-0006 additive; card t_64dff55f).** The `Hoe` clone scaffold is kept ONLY for its equip / `attach`-transform + ItemDrop/equip rigging (the relog-binding sibling t_85f45dd7 depends on it); the Hoe's *visible held mesh* under `attach > visual` (blade `stone` + handle `wood` — the gardening-tool silhouette Daniel flagged) is stripped and replaced by `Assets.BuildFieldMapVisual` → `Assets.BuildFieldMapMesh`, a fresh `new Mesh()` (verts/triangles/uv + `RecalculateNormals`/`RecalculateBounds`) — a few lightly-folded, double-sided quads reading as a flat/folded field map. **This is the FIRST procedural world-mesh authored in the repo** (`grep "new Mesh()" src/` was 0). Skinned with the vanilla **`leatherscraps`** material read as a blueprint (`Assets.TryReadLeatherMaterial` off the `LeatherScraps` prefab — reading a shared material reference is clean-room-safe per ADR-0006; falls back to a default material + LOUD warning if unreadable, shape fix still lands). **Disambiguation is silhouette/value (flat sheet vs handle+blade), NOT hue** (design pillar 2 + Daniel is colorblind). **In-game AT pending** — the held silhouette is an in-hand F-key check on a GPU client; only build (0/0) + clean registration verified (logs-green ≠ playable). |
| Patch surface | `LocalMapEquipPatch` — prefix+postfix on `Humanoid.EquipItem(ItemData,bool)` (overload-disambiguated): the torch exception (C12/AT-MAP-TORCH). `LocalMapBootstrapPatch` — postfix on `Minimap.Start` attaching the client-only `LocalMapController` (carry/equip state machine). **(Issue 10, IMPLEMENTED 2026-06-12, card t_41482aa3)** `LocalMapTooltipNamePatch` (Postfix on private `InventoryGrid.CreateItemTooltip` → overwrites `UITooltip.m_topic`, the title) + `LocalMapHoverNamePatch` (Postfix on `ItemDrop.GetHoverName` → world-drop hover) substitute the bare `sbpr_map_name` (with a `"Map: "` display prefix) for the title — **guarded on presence of the `sbpr_map_name` key** (not `m_dropPrefab`, which is `[NonSerialized]`), pure pass-through otherwise; both `PatchAll`-registered in `Plugin.Awake` (PatchCheck-guarded, AT-TABLENAME-8). Combat suppressed by empty `m_attack`/`m_secondaryAttack` animations (`HavePrimaryAttack`/`HaveSecondaryAttack` false → no LMB/RMB/block — AT-MAP-BLOCKCLEAR). Item built by the repo's clone-and-reshape idiom (donor `Hoe`, like the Spade): type reshaped to TwoHandedWeapon, build PieceTable nulled, attacks emptied, **and the Hoe's visible held mesh (`attach > visual`) replaced by a procedural blank-leather field-map sheet** (see Visual notes; card t_64dff55f) so none of the Hoe's silhouette survives. |
