...
| Render | **HUD overlay** (`Hud.Awake` postfix → MonoBehaviour under `Hud.m_rootObject`), the Iron Compass doctrine (`nomap.md` §8). 🔴 **NOT minimap pins** — the SB server runs NoMap by default (NoMapEnforcer sets `GlobalKeys.NoMap`), so pins have no surface; a HUD overlay works regardless of map state. 🔴 **RENDER REDESIGNED (card t_b8a19487, Daniel 2026-06-18):** the surface is a **camera-relative trophy RING** around the player — each hostile's TROPHY on a fixed-radius screen ring at its bearing-relative-to-facing, size ∝ proximity, ★ pips (`GetLevel()-1`) above star-level enemies, generic glyph for trophy-less hostiles. Camera-relative, NOT north-up (preserves the Iron Compass's exclusive north payoff). Supersedes the shipped bottom-center text placeholder (now an optional `DebugTextReadout`, default off). Spec: `docs/design/sunstone-lens-trophy-ring.md`. |
...
| Status | IMPLEMENTED (code + spec + SpecCheck item row, card t_ee61472f, 2026-06-17). Build 0/0. **logs-green ≠ playable** — AT-COMPASS-CRAFT/EQUIP-GATE/HEADING/LAG/TILT/NOMAP-SAFE/HUD-HIDE/VANILLA-ONLY/ART close only on Daniel's in-game check (needle direction, lag feel, anchor placement are GPU-client checks). |
