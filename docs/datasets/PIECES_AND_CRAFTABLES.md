...
| Function | One buildable free-standing signpost, placed unpainted. Interacting with the placed sign opens a **custom combined Paint+Text uGUI panel** (replaces the vanilla text dialog): pick a **text/board color** AND an optional **border color** (two-tone), pay **one pigment per CHANGED color slot** (delta vs the sign's current ZDO color — §A2.6 per-changed, LOCKED Daniel 2026-06-21) via `{Paint this and consume}` (same color in N changed slots = N of that pigment; border optional; ≥1 changed slot required; unchanged slots + clears free; re-painting re-consumes only CHANGED slots), and edit the label via `{Update Text}` (FREE, locked until ≥1 color is chosen). Each swatch row leads with an explicit **`∅ None`** tile and renders **only DISCOVERED pigments** (no dead reserved slots). The text color tints the **board mesh AND the written letters**. Both colors + text persist + sync via the sign's ZDO. |
...
| Function | Crafting ingredient: consumed at craft time to bind a Cairn Marker's color; consumed at paint time (one per CHANGED color slot — delta vs the sign's current color, §A2.6 per-changed) via the Painted Sign panel to paint/repaint a placed sign. |
| Stack size | 20 |
| Weight | 0.1 |
| Patch surface | None — pure ObjectDB registration |
| Status | SPEC LOCKED |
...
| Visual notes | **Held mesh = procedural FLAT painted-deer-hide rectangle (0.5 × 0.3 m) + build-Hammer hold stance (ADR-0006 additive; ticket-localmap-hoe-model, 2026-06-24 — supersedes the t_64dff55f folded-leather sheet).** The `Hoe` clone scaffold is kept ONLY for its equip / `attach`-transform + ItemDrop/equip rigging (the relog-binding sibling t_85f45dd7 depends on it); the Hoe's *visible held mesh* under `attach > visual` (blade `stone` + handle `wood` — the gardening-tool silhouette Daniel flagged) is stripped and replaced by `Assets.BuildFieldMapVisual` → `Assets.BuildFieldMapMesh`, a fresh `new Mesh()` **flat double-sided quad**. It is skinned by **instancing** the vanilla **`leatherscraps`** material (`new Material(leather)` — keeps the lit shader + leather normal grain) and swapping `_MainTex` to a **generated map-painted-on-deer-hide texture** (`assets/textures/local_map_held_v0.1.png`, authored 160×96 by `scripts/gen_local_map_held_texture_v01.py`, **Point-filtered** for Valheim pixelation; `_Color` forced white to avoid muddy-multiply). The **held STANCE** is the build-**Hammer** pose: `m_animationState = OneHanded` (ground-truthed off the real vanilla Hammer prefab — the in-hand pose is driven purely by `m_animationState`, independent of `m_itemType`), while `m_itemType` stays `TwoHandedWeapon` for the block-clear equip discipline. Falls back to plain instanced leather (texture missing) or a default material (leather unreadable) with a LOUD warning — shape + stance still land. **Disambiguation is silhouette/value + the painted map, NOT hue** (design pillar 2 + Daniel is red-green colorblind: dark ink on tan, no red-on-green). **In-game AT pending** — the held look + pose are an in-hand F-key check on a GPU client; only build (0/0) + 284 tests green + clean registration verified (logs-green ≠ playable). |
| Patch surface | `LocalMapEquipPatch` — prefix+postfix on `Humanoid.EquipItem(ItemData,bool)` (overload-disambiguated): the torch exception (C12/AT-MAP-TORCH). `LocalMapBootstrapPatch` — postfix on `Minimap.Start` attaching the client-only `LocalMapController` (carry/equip state machine). **(Issue 10, IMPLEMENTED 2026-06-12, card t_41482aa3)** `LocalMapTooltipNamePatch` (Postfix on private `InventoryGrid.CreateItemTooltip` → overwrites `UITooltip.m_topic`, the title) + `LocalMapHoverNamePatch` (Postfix on `ItemDrop.GetHoverName` → world-drop hover) substitute the bare `sbpr_map_name` (with a `"Map: "` display prefix) for the title — **guarded on presence of the `sbpr_map_name` key** (not `m_dropPrefab`, which is `[NonSerialized]`), pure pass-through otherwise; both `PatchAll`-registered in `Plugin.Awake` (PatchCheck-guarded, AT-TABLENAME-8). Combat suppressed by empty `m_attack`/`m_secondaryAttack` animations (`HavePrimaryAttack`/`HaveSecondaryAttack` false → no LMB/RMB/block — AT-MAP-BLOCKCLEAR). Item built by the repo's clone-and-reshape idiom (donor `Hoe`, like the Spade): type reshaped to TwoHandedWeapon, build PieceTable nulled, attacks emptied, **and the Hoe's visible held mesh (`attach > visual`) replaced by a procedural flat painted-deer-hide map rectangle + the build-Hammer hold stance** (see Visual notes; ticket-localmap-hoe-model) so none of the Hoe's silhouette or pose survives. |
...
| Render | **HUD-mounted, but WORLD-SPACE visuals** (`Hud.Awake` postfix mounts the client-only host MonoBehaviour; the host stays active for the #209 Update-pump). 🔴 **NOT minimap pins** — the SB server runs NoMap by default (NoMapEnforcer sets `GlobalKeys.NoMap`), so pins have no surface. 🔄 **EIDETIC WORLD-SPACE HALO (card t_68672b6b, Daniel 2026-06-21 — supersedes the screen-space ring of t_b8a19487):** the standalone (no-minimap) surface is a **diegetic head-halo of billboarded trophies floating in the world** around the player — each hostile's TROPHY (`m_icons[0]` flat sprite on a billboarded quad, vanilla `Billboard`/`LookAt`) placed at its **real world bearing** at a **FIXED ring distance** (`pos = eyePoint + dir·HaloRadius` — every trophy equidistant from the eye, no range-dependent push), with **scale carrying all the range info via a 10m knee** (full ≤10m → 0.25 at the 70m detection edge; `scale = Lerp(scaleNear·0.25, scaleNear, 1-Clamp01((dist-10)/(detectRadius-10)))`, bug-fix t_10bacccf supersedes the earlier variable-radius+scale), **vanilla nameplate star pips** (`GetLevel()-1`, harvested from `EnemyHud.m_baseHud`), and **aggro-state tint** (🟡 idle / 🟠 aggroed-other / 🔴 aggroed-YOU via `BaseAI.IsAlerted`/`GetTargetCreature`). Trophy-less hostiles use a **hybrid**: variant→sibling remap table (Greyling→Greydwarf) → generic `threat_fallback` glyph for the rest (never omit); `DumpUnmappedCreatures` (default ON) logs unmapped creatures at startup for review. Empty (worn+charged, nothing near) → a **world-space pulsing sun-corona disc** (card t_9d7c3dfe — `ShowEmptyRing` default ON, repurposed to gate the corona; supersedes the flat screen-space `_emptyRing` annulus of the original /bug t_2d500d45): a glowing gold disc co-located with the trophy halo in the **same scene root**, breathing on a slow ~0.25 Hz alpha pulse (the engine-free CI-gated `SunstoneCoronaPulse`), the substrate the trophies orbit — **`FeetGlow` (default, re-lock 2026-06-24): an upright camera-facing glow rising from the feet, soft at the ground (no hard terrain-clip), bright core blooming to full width by ~0.5m, soft dome above** (engine-free CI-gated `SunstoneCoronaProfile`); live-flippable to the old flat `GroundPlane` "sun on the floor" disc or `CameraFacing`; depleted/not-worn → halo off. **Camera-relative, NOT north-up** (holds by construction — trophies sit on the real `dir`; preserves the Iron Compass's exclusive north payoff). Only renders in the **no-minimap fallback** (`MinimapHandoffMode`); minimap surfaces keep flat icons (separate code, card t_aab051ae). Spec: `docs/design/sunstone-lens-trophy-ring.md` + `docs/v3/planning/sunstone-lens-corona-impl-spec.md`. |
| Visual notes | **Additive (ADR-0006)** — `Assets.ConstructItemShell` builds the networked item skeleton from scratch (NEVER clones a vanilla item). World-drop mesh grafted as a ZNetView-free cosmetic child off the vanilla `Crystal` blueprint (`Assets.GraftVisualSubtree`, child `attach`). Inventory icon `sunstone_lens_v0.1.png` (v0.1 placeholder; icon MANDATORY — crafting UI indexes `m_icons[0]`; `ConstructItemShell` pre-seeds a magenta fallback + SpecCheck C1 screams if the real PNG didn't ship). 🔄 **World-halo render is additive too (card t_68672b6b)** — each slot is a `new GameObject` + `MeshFilter`/`MeshRenderer` (billboarded quad) + vanilla `Billboard` component, **no ZNetView/Piece** (purely cosmetic, client-local; the ADR-0006 "constructed GameObjects carrying only MeshFilter/MeshRenderer" pattern, NOT a prefab clone-and-strip). Trophy + star sprites are READ from vanilla prefabs (reading an asset is not cloning), the sun-corona disc + its radial-glow sprite are code-generated (card t_9d7c3dfe — additive `new GameObject` + world-space `Canvas`/`Image` [+ optional `Billboard`], no `ZNetView`/`Piece`, procedural sprite). **No new asset** — `threat_fallback_v0.1.png` already shipped; the variant→sibling remap table is code/data, not art; trophies reuse vanilla `m_icons[0]` (flat, Knob #4 — NOT the 3D attach mesh); the corona ships zero PNGs. |
| Patch surface | **(1) Harmony Prefix on `Humanoid.DrainEquipedItemDurability(ItemData, float)`** (decomp :13227) — for OUR lens on the LOCAL player only: drains at the fixed rate or recharges in the sun, clamps to [0, max], returns false to SKIP vanilla so the break/unequip/destroy-at-zero branch is NEVER reached (AC#5). Pure pass-through for every other item / non-local player; fails OPEN. **(2) Harmony Postfix on `Hud.Awake`** — mounts the client-only world-halo detection host. Both registered in `Plugin.Awake` (PatchCheck asserts they wove); inert on the dedicated server. Equipped-lens detection via public `Inventory.GetEquippedItems()` + `m_dropPrefab` name (`m_trinketItem` is protected). Halo reads `Character.GetEyePoint`, vanilla `Billboard` (`:99987`), `CharacterDrop`/`ItemType.Trophy`, `EnemyHud.instance.m_baseHud`, `BaseAI.IsAlerted`/`GetTargetCreature`/`Character.GetLevel`, and `ZNetScene.m_prefabs` (the startup unmapped-creature dump) — all public base-game (ADR-0001 clean-side; Rune Magic *behaviour* reproduced from vanilla primitives, no third-party code read or committed). |
| Config | `SunstoneLens` section (live-tunable): `MaxCharge` (100), `DrainPerSecond` (0.33 ≈ 5 min full→empty), `ChargePerSecond` (1.0 ≈ 1.7 min empty→full), `DetectRadius` (70 m), `DetectIntervalSeconds` (0.5), `ClearWeatherNames` (optional allowlist; empty = m_isWet-driven). 🔄 **World-halo render knobs (card t_68672b6b → geometry re-locked by t_10bacccf — replace the screen-space `RingRadiusPx`/`RingCenterOffsetY`/`RingIconMinPx`/`RingIconMaxPx`):** `HaloRadius` (**1.0 m** — **FIXED** ring distance, pulled in from 2.0 m per Daniel 2026-06-24; the old `HaloRadiusMin`/`HaloRadiusMax` collapsed), `HaloScaleMax` (**0.75** — FULL scale at ≤10m, the "1.0", +25% from 0.6 per Daniel 2026-06-24; edge scale is **derived** 0.25× = the "0.25", so the old `HaloScaleMin` is removed), `HaloEyeOffsetY` (~0), `RingMaxIcons` (12, carried), `ShowEmptyRing` (true — **repurposed (card t_9d7c3dfe)** to gate the world-space sun-corona, same `.cfg` key, was the flat ring), `ShowDepletedHint` (false), `DumpUnmappedCreatures` (**true** — startup unmapped-creature dump), `DebugTextReadout` (false), `DebugMount` (true this cut — bake false once confirmed in-game). 🐛 **Sun-corona empty-state knobs (card t_9d7c3dfe → FeetGlow re-lock 2026-06-24, all live-tunable):** `CoronaOrientation` (**FeetGlow** ↔ GroundPlane ↔ CameraFacing — live-flippable; FeetGlow = the default upright rising feet-glow, GroundPlane = the old flat floor disc), `CoronaHeight` (1.2 m — how tall the FeetGlow rises), `CoronaFullWidthHeight` (0.5 m — height it blooms to full width), `CoronaBaseWidthFrac` (0.15 — base narrowness), `CoronaPulseHz` (0.25 — breaths/sec, 0 = steady), `CoronaAlphaTrough` (0.10), `CoronaAlphaPeak` (0.28 — around the 0.18 baseline), `CoronaInnerFill` (0.35 — GroundPlane/CameraFacing radial disc only), `CoronaThickness` (0.45 — soft edge falloff), `CoronaRadius` (~1.0 m — default-tracks `HaloRadius`), `CoronaPlaneOffsetY` (0 — vertical lift). The pulse ENVELOPE is the engine-free CI-gated `SunstoneCoronaPulse` (AT-CORONA-PULSE-MATH); the FeetGlow vertical profile is the engine-free CI-gated `SunstoneCoronaProfile` (AT-CORONA-FEET-PROFILE). |
| Status | IMPLEMENTED (code + spec + SpecCheck item row, card t_2fd7bc7f, 2026-06-16). **Trophy-ring render BUILT** (card t_b8a19487, 2026-06-19). **HUD-render bug FIXED** (card t_d5949685, 2026-06-19 — dead-Update-pump, `_content`-child toggle). 🔄 **RENDER REDESIGN → WORLD-SPACE EIDETIC HALO** (DESIGN card t_68672b6b → IMPL/PR #242 t_d17d9b58, Daniel 2026-06-21 "make the standalone ring eidetic"; head-halo placement, hybrid trophy-less + startup dump, flat billboarded sprites). 🐛 **HALO GEOMETRY RE-LOCKED → FIXED-distance ring + 10m scale knee** (bug-fix card t_10bacccf, Daniel 2026-06-22: PR #242's variable radius+scale pushed far enemies out AND shrank them to invisibility; now a single `HaloRadius` + scale-only range cue, full ≤10m → 0.25 at the edge; CI-gated `SunstoneHaloGeometry` AT-HALO-FIXED-DIST/SCALE-KNEE). 🐛 **EMPTY-STATE → 3D PULSING SUN-CORONA DISC** (card t_9d7c3dfe, Daniel /bug t_2d500d45 "the ring itself is just a screen space circle, not a 3d slowly pulsing 'sun corona' disc": the flat screen-space `_emptyRing` annulus is REMOVED and replaced by a world-space sun-corona disc sharing the trophy halo's scene root, breathing on the CI-gated `SunstoneCoronaPulse`; all knobs reversible live-config defaults). 🐛 **CORONA → FEET-GLOW + RING RE-TUNE** (2026-06-24, Discord `ticket-diegetic-halo-render`: the flat `GroundPlane` corona clipped terrain + read flat → new default `CoronaOrientation.FeetGlow`, an upright rising feet-glow with a soft ground meet, CI-gated `SunstoneCoronaProfile` AT-CORONA-FEET-PROFILE; GroundPlane/CameraFacing kept selectable. Same PR: `HaloRadius` 2.0→**1.0 m**, `HaloScaleMax` 0.6→**0.75**). SpecCheck manifest unchanged (render-only). **logs-green ≠ playable** — the world halo is a visual feature; AT-EIDETIC-1..5/AGGRO/CAMREL/OCCLUDE/PERF/MINIMAP-UNAFFECTED + AT-HALO-FIXED-DIST/SCALE-KNEE + AT-CORONA-3D/ORIENT/PULSE/ART/GATED/SUBSTRATE close only on Daniel's in-game GPU-client check (AT-CORONA-PULSE-MATH + AT-CORONA-BUILD are headless-confirmed). |
...
- **Twisted Portal** (Trailborne v3 Swamp — the endgame no-restriction portal; distinct from the v2 Ancient Portal, which is the convenience portal that KEEPS the ore ban). **SPECCED — see `docs/v3/planning/twisted-portal-impl-spec.md`** (reconciled to food-as-fuel, card t_c15411b2; original t_f9cab392). **One prefab: the `piece_sbpr_twisted_portal`** (Hammer-placed, solid-earth; teleports even with `NoPortals` set; **destination chosen by LOOK-TO-AIM** — stand on a portal, aim the crosshair at a destination portal, see a food-impact preview, press [Use]/E to commit; SUPERSEDES the prior same-rune-name pairing, card t_3d908685, 2026-06-27). Rune names persist in a dedicated `sbpr_rune_name` ZDO slot as human-readable **aim labels** (not the pairing key). **Cost model = FOOD-AS-FUEL (no key item)** per the locked design `docs/design/twisted-portal-food-charge.md` (PR #270/#271): teleport range is gated by belly food — Portal Energy `PE = remaining-food-minutes × a stat-derived tier`, a jump spends food-time scaled by distance (so distance both costs provisioning AND lands you depleted), and **Bukeperries** (vanilla `Pukeberries`) are a burnable emergency reserve spent only on the shortfall (30 m/berry, 10 = the 300 m ceiling; a berry-jump arrives food-empty AND *Feeling Sick*). Consumes the shared v3 **Sunstone** material in its build recipe (second Sunstone consumer after the Sunstone Lens). **Build recipe LOCKED (card t_2b388cd5, Q1 = coexist): FineWood ×20 + GreydwarfEye ×10 + SurtlingCore ×4 + Sunstone ×1 → 1**, Hammer-placed (no station), solid-earth only, HP 300. *(The former `SBPR_TwistedKey` Trinket charge-meter is REMOVED — superseded by food-as-fuel. The "Pocket Portal" idea was rethemed + specced as the v2 **Portal Seed → Ancient Portal** above.)* **Portal MECHANISM impl is in review (card t_2b388cd5, PR #273 MERGED to main — the distinct class, NoPortals bypass, rune-name pairing; in-game accept pending Daniel). The food-as-fuel COST MODEL is now IMPLEMENTED (card t_6e992a30): the `TwistedPortalEnergy` PE engine (read `Player.GetFoods()`, stat-derived tier, distance→food-time debit, Bukeperry shortfall, `SE_Puke` on a berry-jump) hooks C1's `TrySpendForJump` seam, with the six PE knobs as live `TwistedPortal` BepInEx config, a boot-time PE-constant SpecCheck assertion, the lore breadcrumb in the portal description, and a CI-gated engine-free `PortalEnergyMath` truth table (AT-PE-MATH, 58 tests). Build 0/0; logs-green ≠ playable — the AT-PE-DEBIT/ARRIVE-DEPLETED/BUKE-RESERVE/BUKE-SICK/FEAST-CLOCK in-game accepts are Daniel's. The through-terrain overlay (card t_e732bd8b) is the remaining sibling impl card.**
