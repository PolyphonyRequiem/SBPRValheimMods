...
| Config | `SunstoneLens` section (live-tunable): `MaxCharge` (100), `DrainPerSecond` (0.33 ≈ 5 min full→empty), `ChargePerSecond` (1.0 ≈ 1.7 min empty→full), `DetectRadius` (50 m), `DetectIntervalSeconds` (0.5), `ClearWeatherNames` (optional allowlist; empty = m_isWet-driven). 🔄 **World-halo render knobs (card t_68672b6b → geometry re-locked by t_10bacccf — replace the screen-space `RingRadiusPx`/`RingCenterOffsetY`/`RingIconMinPx`/`RingIconMaxPx`):** `HaloRadius` (~2.0 m — **FIXED** ring distance, the old `HaloRadiusMin`/`HaloRadiusMax` collapsed), `HaloScaleMax` (~0.6 — FULL scale at ≤10m, the "1.0"; edge scale is **derived** 0.25× = the "0.25", so the old `HaloScaleMin` is removed), `HaloEyeOffsetY` (~0), `RingMaxIcons` (12, carried), `ShowEmptyRing` (true — faint solar ring), `ShowDepletedHint` (false), `DumpUnmappedCreatures` (**true** — NEW, startup unmapped-creature dump), `DebugTextReadout` (false), `DebugMount` (true this cut — bake false once confirmed in-game). |
| Status | IMPLEMENTED (code + spec + SpecCheck item row, card t_2fd7bc7f, 2026-06-16). **Trophy-ring render BUILT** (card t_b8a19487, 2026-06-19). **HUD-render bug FIXED** (card t_d5949685, 2026-06-19 — dead-Update-pump, `_content`-child toggle). 🔄 **RENDER REDESIGN → WORLD-SPACE EIDETIC HALO** (DESIGN card t_68672b6b → IMPL/PR #242 t_d17d9b58, Daniel 2026-06-21 "make the standalone ring eidetic"; head-halo placement, hybrid trophy-less + startup dump, flat billboarded sprites). 🐛 **HALO GEOMETRY RE-LOCKED → FIXED-distance ring + 10m scale knee** (bug-fix card t_10bacccf, Daniel 2026-06-22: PR #242's variable radius+scale pushed far enemies out AND shrank them to invisibility; now a single `HaloRadius` + scale-only range cue, full ≤10m → 0.25 at the edge; CI-gated `SunstoneHaloGeometry` AT-HALO-FIXED-DIST/SCALE-KNEE). SpecCheck manifest unchanged (render-only). **logs-green ≠ playable** — the world halo is a visual feature; AT-EIDETIC-1..5/AGGRO/CAMREL/OCCLUDE/PERF/MINIMAP-UNAFFECTED + AT-HALO-FIXED-DIST/SCALE-KNEE close only on Daniel's in-game GPU-client check. |
