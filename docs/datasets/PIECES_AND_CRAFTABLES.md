...
| Patch surface | Clone of vanilla `Hoe` ItemDrop + a from-scratch spade-only PieceTable (`Trailblazing`). Six terrain-op pieces: the three `piece_sbpr_path_{narrow,standard,wide}` clone the legacy `path` (`TerrainModifier`) and scale all three TerrainModifier radii; the three `piece_sbpr_replant_{narrow,standard,wide}` clone the modern **`replant_v2`** (`TerrainOp`, the Cultivator's grass op) and scale **only** `TerrainOp.m_settings.m_paintRadius` (level/smooth/raise stay stock `false`) — all at 1.5/3/5 m. **Donor-resolution timing (t_d84d2c17):** `replant_v2` has no `ZNetView` so it is NOT in `ZNetScene` at any phase; the replant variants resolve their donor from the vanilla Cultivator's build `PieceTable` (`Assets.FindOpInToolPieceTable`) and register in the **ObjectDB-wiring pass** (`RegisterReplantVariantsFromCultivator`), not at `ZNetScene.Awake` where path variants register. Fails loud at ERROR if the donor can't be resolved. **Placement-ripple preview:** a `Player.UpdatePlacementGhost` postfix (`PlacementMarkerRadiusPatch`) sizes the aiming ripple to the active op's width (see Patched-vanilla entry below) — client-cosmetic, no effect on the op's real radius. |
| Status | SPEC LOCKED |
| Source spec | `specs/2026-06-03-trailborne-v1/planning/requirements.md` |
