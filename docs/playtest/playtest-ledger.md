...
> These are the remaining `src/**/*.cs` changes in the `v0.2.40-playtest..main` window that Round 1 did not cover:
> the Trailside Camp triad completion (bedroll + camp fire + tent collider fit), two Daniel-greenlit cleanup commits
> (cairn banner-diagnostic default-OFF + dead sign-helper removal), and the **three-slice engine-free Core seam**
> (P0/P1/P2). The Core slices are **structural/behaviour-preserving by their own commit messages** — additive
> project seam, ZDO-component migration of `AncientPortalTag`, and a recipe-registry unification of the SpecCheck
> manifest — so their in-game accept is a **boot/smoke confirm** (server boots, SpecCheck logs the same recipe-match
> line, an Ancient Portal still plants+grows+resumes-on-relog) rather than a new player mechanic. **NONE are in-game
> verified yet** — "logs-green ≠ playable" applies to every row, including the "no behaviour change" ones (a green
> build that fails to load a second DLL is still a broken client).
>
> **Ledger identity note (load-bearing):** the three `feat(core)` commits (`adbabf5`/`4413841`/`89962ba`) were
> pushed **directly to `main`** outside the PR flow — they carry **no `t_` card id** and **no trailing `(#NNN)`**,
> and have **no associated GitHub PR** (the `commits/<sha>/pulls` API returns empty). The prepare-playtest / guide
> classifiers key a change's ledger identity off the **commit itself** (card id, else own PR number), so neither a
> card-id nor a PR-number entry in this ledger can represent them. `scripts/prepare-playtest.py` was therefore
> extended with a **SHA-rescue** (mirroring the existing direct-`/bug` PR-number rescue that #264 got): a commit
> that carries neither a card id nor a `(#NNN)` is ledgered iff its **own commit SHA is named in PENDING** below.
> A commit SHA is ground-truth identity that cannot be forged in the ledger, so naming it is an audit trail, not a
> bypass; the rescue is strict to no-card-id commits (an id-carrying commit whose card id is absent stays flagged).

| # | Feature | Card | Status | What to verify in-game |
|---|---------|------|--------|------------------------|
| 35 | **Trailside Camp — Bedroll + Camp Fire (finish the triad: night-skip sleep half + heat-keeping fire half)** | t_439f2351 (#345) | ✅ merged to `main` (`81d504e`); ships in first #7 build | Completes the Trailside Camp triad alongside the tent (item 37). Both **Spade-placed Black Forest** pieces on the **'Trail'** build tab. **Bedroll (`piece_sbpr_bedroll`):** place it, lie down at night → you **skip to morning WITHOUT overwriting your home respawn point** (the load-bearing divergence — a no-spawn night skip; vanilla `Bed.Interact` can't do this, so `BedrollTag` reimplements the 5-gate sleep chain: time/enemies/relaxed-exposure/fire/wet). **(a)** free vanilla **SE_Rested** comfort rides the skip-wake; **(b)** the relaxed exposure gate still needs **underRoof** (a bedroll under open sky is refused) but drops the 0.8-cover clause; **(c)** 🔴 regression: sleeping in a **vanilla** bed still sets spawn as normal (AT-BEDROLL-VANILLA). **Camp Fire (`piece_sbpr_camp_fire`):** place + light it → it **emits heat** (warm status when near, unlike a heat-stripped cairn — AT-BEDROLL-NOFIRE toggles heat with lit state); burns **Wood**; ordinary rain behavior (no mini-roof). **Spec+manifest moved same commit** (SpecCheck +2 Piece rows, +2 dataset rows). **AT-BEDROLL-* / AT-CAMPFIRE-*** per PR #345. logs-green ≠ playable — closes t_439f2351 (defects 2,3). |
| 36 | **Bear Hide Tent — walk-under shelter collider fit (open MeshCollider seated to the measured canopy mesh)** | t_c96a2ea2 (#341) | ✅ merged to `main` (`4bbe862`); ships in first #7 build | Daniel on `v0.2.40`: *"the collision mesh has no relationship to the tent mesh / I am not finding a spot where I can get shelter."* The as-built used a solid box that walled off the interior and a canopy rendered ~4.7 m off it. Fix: graft the donor's **open-sided concave MeshCollider** (walk-under, non-leaky so it still counts as **underRoof** cover) and **seat both the visual and collider to the measured mesh extent** so the canopy foot lands at ground and coincides with the collider; the shell box is demoted to a thin ground pad. Place a **Bear Hide Tent** (Spade, 'Trail' tab, BF tier) and **(a)** walk **under** the canopy — you fit and stand inside (no invisible wall filling the footprint); **(b)** standing under it grants **shelter/underRoof** (rain-cover / relaxed-exposure for the item-35 bedroll seated beneath it); **(c)** the canopy visual **coincides** with where collision actually is (no ~4.7 m offset). Collider geometry only — no SpecCheck/manifest change. **AT-TENT-COLLIDER-FIT** per PR #341. logs-green ≠ playable — closes t_c96a2ea2 (also refs t_439f2351 / t_ea0072ba). |
| 37 | **Cairns — BannerDiagnostic default OFF in shipped builds (release perf, behaviour-preserving)** | t_0027b296 (#342) | ✅ merged to `main` (`2d67e0e`); ships in first #7 build | CLEANUP 3/3 T2-3 (Daniel-greenlit). Flips `CairnTag.DefaultBannerDiagnostic` true→false so the per-frame banner-diagnostic probe **no longer attaches to every cairn banner in release** — behaviour-preserving for players (kills release-only diagnostic overhead). The probe + `sbpr_bannerdiag` console command + `SBPR_BannerDiagnostic` config toggle **remain** (re-enablable). Verify: build a **cairn with a banner** → it looks/behaves exactly as before (no visible change); the log **no longer** shows the per-frame banner-diagnostic spew by default. Chiefly a **no-visible-regression** check (a perf/quality change, not a new surface). Build 0/0, tests 859/859. logs-green ≠ playable — part of t_0027b296. |
| 38 | **Signs — remove dead PinTypes/PinTypeForColor/ColorForPigment helpers (pure dead-code removal)** | t_0027b296 (#340) | ✅ merged to `main` (`918707a`); ships in first #7 build | CLEANUP 3/3 T1 (Daniel-greenlit). Removes three sign helpers with **zero call sites** on main (verified `git grep`): `Signs.PinTypeForColor`, the `Signs.PinTypes` dict, and `Signs.ColorForPigment` (the live `PigmentForColor` is **kept**). **Wire-contract safe** — no ZDO keys / prefab names / config keys touched. Chiefly a **no-regression** check: **Painted Signs still paint + tint + persist exactly as before** (the live `PigmentForColor` path is untouched); nothing player-facing should change. Build 0/0, tests 859/859. logs-green ≠ playable — part of t_0027b296. |
| 39 | **Core seam P0 — stand up the engine-free `SBPR.Trailborne.Core` project (additive, byte-identical rename)** | direct push — no card/PR; ledgered by SHA `adbabf5` | ✅ merged to `main` (`adbabf5`); ships in first #7 build | Arch review P0. **Additive + reversible, zero behaviour change:** a new engine-free `SBPR.Trailborne.Core` project (the boundary enforced by the build — Core references no Unity/Valheim/BepInEx/Harmony), with `BoundedMapMath.cs` **git-mv'd** into it namespace-unchanged (byte-identical body; not one of its ~14 shell callers changes a using). **Load-bearing deployment check:** the modpack now ships a **second DLL** (`SBPR.Trailborne.Core.dll`) — without it the shell throws `FileNotFoundException` at BepInEx load (a green build that crashes on start). Verify: install this build, launch → the client **loads clean** and logs `Loading [SBPR Trailborne 0.2.41]` + `Harmony patches applied.` with **no `FileNotFoundException`** for the Core DLL, and the local-map / cartography surfaces (which use `BoundedMapMath`) still render. No runtime code delta, but the two-DLL load wants one in-game confirm. logs-green ≠ playable. |
| 40 | **Core seam P1 — ZDO-component seam + migrate AncientPortalTag (Model A), behaviour-preserving** | direct push — no card/PR; ledgered by SHA `4413841` | ✅ merged to `main` (`4413841`); ships in first #7 build | Arch review P1. Puts the first real domain consumer behind the P0 Core: the ghost-guard + owner-claim ZDO dance and the portal grow-progress rule, now unit-tested engine-free. `AncientPortalTag` derives from the new `ZdoComponent` base and calls the extracted `PortalGrow.Progress`; **`SBPR_PortalPlantTime` ZDO key UNCHANGED** (R3, no wire change). **Behaviour-preserving** — the accept is an Ancient Portal smoke: **(a)** plant an **Ancient Portal** → it is inert and **scale-lerps ~15 s**, then activates **once**; **(b)** **relog mid-grow** → it **resumes correctly** (absolute world-time, not restarted). 🔴 If a planted portal fails to grow, activates twice, or restarts its timer on relog, the seam regressed. +21 unit cases (was in-world plant+relog, now headless). logs-green ≠ playable. |
| 41 | **Core seam P2 (sliced) — recipe registry retires the SpecCheck↔wiring drift (read-only unification)** | direct push — no card/PR; ledgered by SHA `89962ba` | ✅ merged to `main` (`89962ba`); ships in first #7 build | Arch review Model B, sliced. Builds the single engine-free source of truth for the declarative recipe/piece manifest (`ContentRegistry`) and **projects `SpecCheck.Manifest` from it** instead of a hand-copied literal, so the boot check and the intended shape **can no longer drift** (the 2026-06-03 meta-bug). **Row-set is IDENTICAL** to the old manifest (10 item recipes + 7 pieces, same identifiers); no wire-string minted; every referenced value is `const` (compile-inlined). Does **NOT** yet rewrite the live `DoObjectDBWiring` methods — deferred. **Boot smoke:** start a server → **SpecCheck logs the SAME "checks All N recipes match" line as before** (the manifest now *is* the registry, so it must still pass boot with no assertion failure). Chiefly a **boot-consistency** check — no player mechanic changed. +7 registry-invariant tests; tests 565/565. logs-green ≠ playable. |
...
- **The full `v0.2.40-playtest..88a05e8` (`main`) window has 14 scoped `src/**/*.cs` changes** (Round 1 items 29–34
  + Round 2 items 35–41 + one EXEMPT docs commit). All 14 non-exempt map to a PENDING row → `python3
  scripts/prepare-playtest.py --ref main` reports **0 unledgered**.
- **Round 1 (items 29–34)** — the Twisted Portal look-to-aim rework + Seer's Stone re-lock + cartography/wisp fixes:
  **#286** (`c2ec36d`, t_9e6a0654, item 34), **#287** (`5b2e8f6`, t_2110193e, item 33), **#289** (`e3eb529`,
  t_f4d0d5e1, item 29), **#290** (`570f25b`, t_d3768adf, item 30), **#291** (`8713617`, t_ccb454f8, item 31),
  **#292** (`5ce4489`, t_d9ea1b2c, item 32).
- **Round 2 (items 35–41)** — the Trailside Camp triad + cleanup + the engine-free Core seam:
  **#345** (`81d504e`, t_439f2351, item 35), **#341** (`4bbe862`, t_c96a2ea2, item 36), **#342** (`2d67e0e`,
  t_0027b296, item 37 — `perf(cairns)`, NOT an exempt type so it DOES need a row), **#340** (`918707a`,
  t_0027b296, item 38 — `refactor(signs)`, likewise non-exempt), and the three **direct-to-`main`** `feat(core)`
  commits **`adbabf5`** (P0, item 39), **`4413841`** (P1, item 40), **`89962ba`** (P2, item 41). The three
  `feat(core)` commits carry **no `t_` card id, no trailing `(#NNN)`, and no associated GitHub PR** (pushed outside
  the PR flow), so they are ledgered by the **SHA-rescue** — each row names the commit's own SHA, which the
  extended `prepare-playtest.py` classifier accepts as ground-truth identity (see the Round 2 identity note).
- **EXEMPT in the window:** **`da11e6b`** (`docs(sunstone): fix MinimapBlipStyle default-drift`) is a `docs:` commit
  — no player-visible surface, no test row owed.
