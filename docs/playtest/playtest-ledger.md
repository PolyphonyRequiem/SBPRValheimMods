...
- **`src/**/*.cs` changes on `main` since `v0.2.31-playtest`: 3 commits** — #218 (`f6456ed`, card
  t_91e86951), #220 (`ec057b1`, card t_1afb94cd), and the Painted Sign MaterialMan/MPB tint fix
  (card t_f3310406, #224) — **all three seeded above** as PENDING items 3, 4 & 6. The
  §2L.7-R cursor narrowing (#222/#223) was **reverted on `main` (#225)** as a wrong-direction fix,
  so it carries NO pending ledger entry. The only non-gameplay post-tag change is the installer SHA
  pin **#219** (`a8561f9`) — a release chore, not a gameplay surface. So the auto cross-check is **clean (0 unledgered)**.
...
- `scripts/gen-playtest-guide.py --ref main` confirms **3** code changes / **0** unledgered for this
  window (it diffs `v0.2.31..main`; the #218/#220/#224 card ids are all in the PENDING rows above).
