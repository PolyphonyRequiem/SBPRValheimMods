...
> read or copied. SBPR lines are `main` @ `70ab72b` (every cited line re-grounded against
...
## 2. The gate + the grounding — re-grounded against main @ 70ab72b
...
🔵 **No easing/breathing helper exists anywhere** (grep-confirmed against `main @ 70ab72b`:
...
| Site (`main @ 70ab72b`) | Today | This card |
|---|---|---|
...
via `EnsureLayer()` (`:70-85`) — the blip pool lives there. Add **one** centred gold rim annulus as
a sibling of the blip pool (same `_layer` parent, or a dedicated child of `m_pinRootSmall`):

> 🔴 **Name collision warning — `#238` (t_aab051ae) just shipped a *different* "rim" feature.**
> The threat layer now clamps off-window threat blips to the map edge as smaller dot indicators —
> `RimBlipScale` (`:62`), `RimInset` (`:63`), `Cartography.BoundedMapMath.ClampToRimPx` (`:113`).
> That is an **off-edge blip indicator**, NOT a continuous aura. The lens aura rim is a separate,
> net-new element: a single radially-symmetric gold annulus drawn at the map perimeter regardless of
> threats. **Do NOT reuse the `Rim*` blip-clamp names** — call the aura element `_auraRim` /
> `RenderAuraRim` to keep the two unambiguous. They coexist (one is the threat-clamp, one is the
> "lens is live" cue); neither touches the other's geometry.

```csharp
// In SunstoneMinimapThreatLayer — a single pooled aura annulus, built lazily like the blip pool.
// Mounted under the SAME m_pinRootSmall frame the blips use; the annulus is the RingSprite()-style
// procedural ring (a thin gold band). NORTH-UP, NO counter-rotate (the layer header :35 lock:
// "DO NOT counter-rotate this layer — that is the disc's behaviour, wrong here").
private Image? _auraRim;
internal void RenderAuraRim(float alpha)     // called from the overlay when the vanilla map is the aura home
{
    if (!EnsureLayer() || _layer == null) return;
    if (_auraRim == null) { /* new GameObject + Image, RingSprite() annulus, raycastTarget=false,
                               sized to the small-map diameter, centred at (0,0) */ }
    _auraRim.gameObject.SetActive(true);
    _auraRim.color = new Color(CSolarRing.r, CSolarRing.g, CSolarRing.b, alpha);  // shared phase, §4
}
```

🔴 **Orientation lock (design §1.2 + the layer's own header `:24-35`):** mount the rim **centred,
north-up, NO counter-rotation** (`:35` explicit). A rim is radially symmetric so orientation is
visually moot — but do NOT add the carry-disc's counter-rotate idiom here (wrong surface). Sized to
the small-map diameter (read off the layer's existing geometry, NOT a literal). `Clear()`
(`:160-168`) must also hide `_auraRim` so an unequipped/depleted lens leaves no stale rim.
**Client-only by construction** (`Minimap.instance` null on a dedicated server — header `:37`).
...
**Build it additively, mirroring the shipped `BuildNorthLayer` (`:1799`) — but on the NON-rotating
`_frame`, not the rotating `_mapContainer`:**

```csharp
// MapSurface — a new BuildLensAuraRing(), called from EnsureBuilt beside BuildNorthLayer() (:1625).
// Unlike the N-glyph (which orbits on _mapContainer), the aura ring is RADIALLY SYMMETRIC, so it
// parents to the FIXED _frame (like the bezel itself, :1614-1615) — no orbit, no counter-rotate.
...
N-glyph use, `BuildNorthLayer:1831`) so it tracks any future margin tune and works at BOTH the disc
...
- **Design (LOOK-LOCKED):** [`../../design/sunstone-lens-aura.md`](../../design/sunstone-lens-aura.md)
  (card `t_acaa0190`, PR #235 @ main `70ab72b`).
...
- **Sibling cosmetic (north, not detection) — SHIPPED, source of the live `_bezel.color` collision:**
  ...
  `_bezel.color` `:1244`, `BuildNorthLayer` `:1799`, `SetCompassNorth` `:246`, `MapViewer` fan-out
  `:131-135`.
...
- Card `t_e4a6f559` (graduates `t_acaa0190`). Grounded against `main` @ `70ab72b` (every cited line
  read directly).
