...
**On the limit of what sweep can promise.** A literal "credentials cannot outlive the run
that minted them, even when SIGKILLed" is **not achievable** — SIGKILL runs no handler,
no `atexit`, no `finally`. Claiming it would be exactly the "logs green ≠ playable"
dishonesty `AGENTS.md` prohibits. Three distinct mechanisms exist and should be named
separately rather than blurred into one guarantee:

1. **Graceful teardown** — removal at exit, on the paths the runner actually reaches.
2. **Next-entry sweep (#455)** — no credential survives *into the next run*. This is the
   strongest guarantee available and is what #455's acceptance criterion should say. It
   does not bound the residue window *between* runs.
3. **TTL** — the only always-on bound. A SIGKILL-orphaned bootstrap doc is
   cryptographically inert past `expiry_unix_ms`, enforced by the C# arm gate.

Note the residual gap: **lane-password files carry no TTL**, so their validity ends only
with lane teardown. That is the real exposure the "~113 minutes" observation describes,
and #455 does not close it; shortening the disposable lane's own lifetime is #457's
scope. A supervising watchdog would bound the window without an intervening `arrange`,
and is deliberately *not* folded into #455 — it needs a supervision-topology decision of
its own.
...
**P9 — Proof seams are mandatory, not defaulted.** Where a check's value rests on
establishing a fact the caller must supply the capability for — proving a **negative**
(a component is absent, a tree contains no such file), or reading an artifact whose
content is the evidence (a launch wrapper) — that capability must be a required
dependency. A defaulted "cannot prove" seam is not a security hole when it fails closed,
but it is a diagnostic one: an omitted wiring is then reported as a fault in the
*client's filesystem* rather than as an incomplete caller, sending an operator to
inspect a machine that is fine. Make the incomplete caller impossible to construct
instead.

Applies to **every** `StaticEnvironment` seam, not only the enumeration one. `#454`
established the rule for `find_named_files`; overlapping merges re-defaulted it and
`#467` restored it and extended it to `read_text`, which carries the #453 join-delivery
proof and has the identical failure mode. Wiring a proof seam is a decision, so it is
made at the construction site. (#454, restored and generalised by #467.)
...
SWEEP    (idempotent cleanup of prior-run residue)                  [#455]
  ...
  └── no credential survives INTO the next run; see I10 on why a
      hard "cannot outlive a SIGKILLed run" guarantee is unachievable
