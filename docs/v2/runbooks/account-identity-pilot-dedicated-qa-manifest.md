...
| Standing admin Steam id | `<withheld — operator adminlist by path>` | `/config/adminlist.txt` |
...
- The QA-lane admin Steam id is withheld from this manifest; it appears in `adminlist.txt` by
  necessity — treat the adminlist file as operator-controlled and reference it by path in evidence,
  do not paste any raw Steam id into narrative artifacts.
