...
> **Remediation status (t_79588427, T021 remediation 2):** the runtime develop/
> purchase ingress this FAIL identified as missing has since been added — see
> `T021-REMEDIATION-2-PROVISIONING-RUNTIME.md` in this folder. This FAIL evidence
> is RETAINED verbatim as the decision-grade record of the defect it caught; it is
> marked `historical` because the specific gap (zero runtime callers for
> `LocalNodeProvisioningDriver` / `PurchaseCommandHandler`) is now closed. The
> joined-client effective-Level-3 artifact is re-run downstream (QA `t_8261a415`).
