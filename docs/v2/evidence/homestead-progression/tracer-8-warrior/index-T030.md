...
| R9 | Runtime seam postfixes the PRIVATE `Player.QueueEquipAction`/`Player.QueueUnequipAction` (decomp `Player.cs` :6935/:6960) and scales the just-appended `Player.m_actionQueue` copy; activation fail-closed (host derive / client personal cache). **T030 remediation**: the accepted PR #376 bound to `Humanoid` (which declares neither method) so patch discovery resolved zero methods and the effect never fired in-world (QA `t_2b1e690d`); rebound to `Player` verified against `assembly_valheim.dll` metadata | `src/SBPR.Niflheim.HomesteadStones/Features/Warrior/ReadyHandsEquipDurationPatch.cs` |
| R9b | CI regression guard: the reflection apiprobe fails the build if `Player.QueueEquipAction(ItemDrop.ItemData)`, `Player.QueueUnequipAction(ItemDrop.ItemData)`, or `Player.m_actionQueue` stops resolving on the live assembly. Non-tautological: verified locally that the old `Humanoid` binding resolves ZERO methods while the new `Player` binding resolves all three | `.github/workflows/ci.yml` (Verify reflection targets resolve) |
| R9c | **SECOND, distinct runtime-binding failure of the same effect (ADO #125), separate from the R9 `Humanoid` misbind.** The corrected `Player`-bound patch class was never handed to `harmony.PatchAll` in `Plugin.cs` `Awake()`, so HarmonyX wove neither postfix and Ready Hands was inert in-world with all 10 tests and the R9b apiprobe guard green — R9b proves the TARGET resolves, never that the patch class was REGISTERED. `git grep ReadyHandsEquipDurationPatch` returned zero `src/` call sites. Fixed by adding `harmony.PatchAll(typeof(Features.Warrior.ReadyHandsEquipDurationPatch));` with an inline load-bearing-registration comment; `EquipDurationProvider.cs` untouched. Third instance of the family (R9 misbind; IAP-015 dead operator classes at `Plugin.cs:90-98`); the general boot-time weave guard is ADO #126, not this card. In-world timing remains UNPROVEN | `src/SBPR.Niflheim.HomesteadStones/Plugin.cs` (Warrior registrations); `docs/v2/planning/homestead-stone-progression-tasks.md` (T030) |
| R10 | Full suite 1375/1375; both net48 Release builds 0w/0e; docs-lint OK 207; `git diff --check` clean; SpecCheck recipe count unchanged | build/test logs (this run) |
