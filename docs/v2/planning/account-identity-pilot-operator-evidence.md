...
| `AT-AIP-DELETE-DRAIN-BARRIER` | delete drains, commits `DeletionPending` + revokes every linked credential AND allowlist entry in one terminal transaction, closes the session; a same-subject re-join cannot recreate the account (rejected — no active allowlist) |
