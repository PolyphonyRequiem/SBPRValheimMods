...
- **Clean-side → `engineer-ui`** (owns `MapSurface.cs` + the cartography viewer cluster). Render +
  ...
  > **SUPERSEDED by §2M (card t_b5e535b0):** the "new `SurveyPin` field + WireVersion bump" this note
  > anticipated turned out to be the WRONG mechanism. §2M rides the existing transient threat-layer seam
  > instead, so the location layer persists nothing — **no `SurveyPin` field, no WireVersion bump.** Read
  > §2M, not this sentence, for the location-pin design.


### 2M — Vanilla location/POI pins on the SBPR local map — LIVE re-derive (card t_b5e535b0)

> **Group-2 follow-up to §2K (Group-1 boss/Hildir pins, PR #263).** Design LOCKED by Daniel: the full
> auto-icon set, live re-derive, both surfaces. Shipped in `LocationPins.cs` + a `MapSurface.RebuildOverlay`
> layer. Final visual accept is Daniel's in-game GPU client (AT-LOC-* below) — the headless box collapses
> shaders and cannot confirm the on-screen render.

**What this adds.** Vanilla location/POI icons on the SBPR local map — Haldor's vendor, StartTemple,
Hildir's camp, the BogWitch, discovered POIs, and any **modded** flagged location — on BOTH surfaces (the
...
identical for every viewer** — there is no per-viewer fog gate, no holder-vs-imprinter ambiguity
...
#### 2M.2 The boss-vs-location split (why Group-1 ≠ Group-2)

- **Boss / Hildir pins (Group-1, §2K)** are **player-local, `save:true`** map pins — they live in the
...
#### 2M.3 Design B — ride the transient threat-layer seam (NOT a synthesized `SurveyPin`)
...
re-derive → transient icon → render without persisting". The location layer is built as a **sibling of the
threat layer**, which buys four properties **by construction**:

- **Icon-only** — the collector returns only `(pos, icon)`; the spawn path has no label code, so K-C (no
...
- **Atlas-safe** — the renderer uses `Image.sprite` + `preserveAspect` (Unity-native atlas handling), so
  the §2K.7 `uvRect`-crop concern never arises.
- **Non-deletable + no-wire-bump** — location markers are **not** `SurveyPin`s, never enter
...
- **Zero textual conflict with Group-1** — the layer touches none of `ResolvePinSprite`, `SpawnPinMarker`,
  `CollectShareablePins`, or `RemovePinNear`; the only shared region is one additive loop in
  `RebuildOverlay`.

#### 2M.4 K-A — derive pipeline (all-public, no reflection)

In `LocationPins.Collect(into)`:

1. Guard `Minimap.instance == null || ZoneSystem.instance == null` → empty (mirrors the WorldPins
...
2. `ZoneSystem.instance.GetLocationIcons(dict)` → `pos → prefabName` for the filtered server set.
3. For each `(pos, prefabName)`, resolve the sprite by **mirroring private `Minimap.GetLocationIcon`**
   ...
   miss yields `default` → `m_icon == null` → **skip that location** — exactly as vanilla's
...
#### 2M.5 K-B — render representation (sibling of the threat layer)
...
- `SpawnLocationMarker(icon, anchored)` — mirror of `SpawnThreatMarker`: `Image`, `img.sprite = icon`,
  `img.preserveAspect = true`, `img.raycastTarget = false`, `img.color = Color.white` (location art is
  self-coloured — do NOT tint). Added to `_pinObjects` so it counter-rotates upright (`CounterRotatePins`)
...
pins (`UpdateLocationPins:1175`), so the default starts above the survey-pin `PinIconPx` (22) at ~1.4×.
This is a one-line **calibration knob for Daniel's in-game eyeball** — the headless box can't judge it.

#### 2M.6 K-C — label policy: ICON-ONLY

Structurally enforced by §2M.5: the spawn path has no text child, and `LocationMarker` carries only a
sprite. The `GetLocationIcons` value is a raw prefab name (`Vendor_BlackForest`, `StartTemple`), never
display text — and it never reaches a renderer. (Prefab-name → localized-name labels would need a name map
and are **out of scope** unless Daniel asks.)

#### 2M.7 Both surfaces — already covered

`RebuildOverlay` is the single overlay builder for BOTH the carry-disc (`_cfg.PlayerCentred=true`,
`ShowPrompts=false`) and the Surveyor's-Table modal (`PlayerCentred=false`). One loop → both surfaces.
Unlike the threat layer (disc-only — it's a threat radar), the location layer runs on the modal too:
locations are **navigation landmarks**. The carry-disc rebuilds every `LocalMapController.PollSeconds=0.25f`;
the modal rebuilds on open/refresh. **Live pop-in is free** — no new timer (**AT-LOC-3**).
...
This card does NOT change the bound behavior — unbounded location pins are **out of scope** (only Daniel
changes the bound).
...
- **AT-LOC-1** — Haldor's vendor pin appears on BOTH surfaces (modal + carry-disc) with vanilla pin art,
  within the table's 1000 m window.
- **AT-LOC-2** — Other auto-icon locations (discovered POIs / StartTemple / Hildir camp / BogWitch, when
  in-window) appear with their vanilla icons.
- **AT-LOC-3** — New locations pop in live as discovered (≤ ~0.25 s on the carry-disc; on modal refresh) —
  not frozen at imprint.
- **AT-LOC-4** — Location pins are ICON-ONLY — no raw `Vendor_BlackForest` / prefab-name label.
- **AT-LOC-5** — The TableEdit eraser cannot delete a location pin (`raycastTarget=false`; not a
  `SurveyPin`). An adjacent player marker-sign pin still deletes normally.
- **AT-LOC-6** — No regression: SBPR MarkerSign pins + Group-1 boss/Hildir pins + Sunstone threat blips all
  still render (this only ADDED a layer). *(303/303 structural tests pass; `WireVersion` grep-asserts `1`.)*
- **AT-LOC-7** — A shared/imprinted disc shows the same location set to every viewer (server-global).

#### 2M.11 Routing (this amendment)

- **Clean-side → `engineer-ui`** (owns `MapSurface.cs` + the cartography cluster). Spec + code in ONE PR
  (AGENTS.md spec/code co-movement). New file `LocationPins.cs`; additive `RebuildOverlay` layer +
  `SpawnLocationMarker` + two knobs (`LocationPinPx`, `_locationScratch`). `dotnet build … -c Release` =
  0W/0E (`TreatWarningsAsErrors` ON). **No** `SurveyPin` field, **no** WireVersion bump — supersedes the
  §2K.11 deferral note's anticipated mechanism.
