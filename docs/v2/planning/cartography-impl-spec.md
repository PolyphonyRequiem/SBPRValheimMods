...
### 2N — Boss/Hildir pins LIVE-derive onto the holder's own map (the missing capture path) — card t_2110193e

> **Group-1 follow-up to §2K (boss/Hildir RENDER, PR #263) — a DIFFERENT defect.** §2K fixed how a boss pin *renders* (icon + localized label + non-deletable) ONCE it reaches the renderer. This fixes that a boss pin **never reaches the renderer** under the common lifecycle "discover a boss, don't survey at a Table since." Design LOCKED by the architect (2026-06-27); final visual accept is Daniel's in-game GPU client (AT-VPIN-LIVE-* below) — the headless box collapses shaders and cannot confirm the on-screen render.

**The bug (Daniel, 2026-06-27, v0.2.40-playtest):** *"I tried re-using the boss stone, and the pin doesn't reappear, but it does say already pinned, so that's a bug…"* A discovered boss/Hildir pin does NOT appear on the SBPR local map, and re-activating the boss stone cannot surface it.

#### 2N.1 Root cause — the capture/render asymmetry (grounded on `main`, file:line)
...
1. `survey.Pins` (`:699`) — the FROZEN blob. **The only source that carries boss pins.**
2. `WorldPins.CollectInDiscPins` (`:702`) — live, but **MarkerSign ZDOs only** (`WorldPins.cs:308-324` scans `MarkerSigns.MarkerTypes`; a boss pin is not a MarkerSign → excluded).
3. `ThreatMarkers` (`:743`) — live, Sunstone, disc-only.
4. `LocationPins.Collect` (`:786`) — live, server-global Haldor/POI (§2M; works).

The **only** writer of boss pins into `survey.Pins` is `SurveyorTableTag.CollectShareablePins` (`SurveyorTableTag.cs:543-558`), called solely from the Table acts `ContributeLocalSurvey` (`:372`) and `IngestBoundCarriedMaps`. It reads vanilla `m_pins`, keeps `m_save && type != Death` (`:552-553`) — boss pins qualify — and bakes them into the persisted survey. **Nothing else captures them.**

Why Daniel's symptom follows: re-using the boss stone is a pure-vanilla `DiscoverLocation`/`AddPin` action that never touches the SBPR survey system; "already pinned" = vanilla refused to re-add a pin already in `m_pins`; and bosses have no live-derive path (sources 2/3/4 all exclude them). The pin demonstrably EXISTS in vanilla `m_pins`, but there is no route from there onto the SBPR map except a Table survey. **The boss stone is pure-vanilla and is NOT touched by this fix** — the fix makes its re-use moot (AT-VPIN-LIVE-2).

#### 2N.2 The boss-vs-location split (why this is a §2K sibling, NOT a §2M sibling)

The ticket's suggested shape — "mirror `LocationPins.Collect` + `SpawnLocationMarker`, render icon-only like the threat/location layer" — is the **WRONG sibling.** Bosses are the mirror image of locations:

| | Boss/Hildir (this §2N) | Location/POI (§2M) |
|---|---|---|
| vanilla pin kind | player-local, `save:true`, in `Minimap.m_pins` | server-global, `save:false`, regenerated by `UpdateLocationPins` |
| label | **REQUIRED** ("Eikthyr") — AT-VPIN-2 already accepted | icon-only by design (AT-LOC-4) |
| dedup vs frozen | **REQUIRED** (a boss can be BOTH frozen+live) — K1 | none (locations are never frozen) |
| source access | **private `m_pins`** → reflection (no public accessor) | all-public (`GetLocationIcons` + `m_locationIcons`) |
| render path | the EXISTING `SpawnPinMarker` (icon+label+size) | `SpawnLocationMarker` (icon-only) |

Mirroring `SpawnLocationMarker` would (a) **drop the boss label** on the modal — a regression vs the frozen boss pin's §2K.2 behavior; (b) render live bosses at `LocationPinPx` (31) with no label while frozen bosses render at `PinIconPx` (22) with a label — a surveyed boss would look different from an unsurveyed one; and (c) require building a NEW dedup against the frozen set. **Correct shape: mirror `WorldPins.CollectInDiscPins`** — emit `SurveyPin`s and `AddIfNew` them into the same `rendered` list, so they route through the EXISTING `SpawnPinMarker` and inherit the §2K.7 boss icon, the §2K.2 localized label, `PinIconPx` sizing, and K1 dedup **for free**, pixel-identical to a frozen boss pin. This is also *less* code than the icon-only path.

#### 2N.3 K-A — derive pipeline (reflection on private `m_pins`)

New file `Features/Cartography/SystemPins.cs`, a Cartography-owned collector that emits `SurveyPin`s (the `WorldPins.CollectInDiscPins` output contract), reading the **private** `Minimap.m_pins` via the cached-`FieldInfo` reflection idiom `SurveyorTableTag.ReadPins` (`:626-631`) already uses (`m_pins` has no public accessor — confirmed decomp `Minimap.cs:289 private List<PinData> m_pins`):
...
The filter `m_save && IsSystemPin(type)` mirrors `CollectShareablePins` exactly, so the live set is the same shape as the frozen set for the same pins (clean K1 dedup). `IsSystemPin` (Boss/Hildir1–3) is the precise discriminator — vanilla pin-UI only ever places Icon0–4, so a manually-placed pin is never caught (§2K.8 comment). **Reuse the SINGLE discriminator:** promote `SurveyData.IsSystemPin` from `private static` to `internal static` so the live-collect filter and the §2K.8 delete-guard can NEVER drift (one source of truth for "what is a system pin"). All field names (`m_save`, `m_type`, `m_pos`, `m_checked`, `m_ownerID`, `m_name`) and the `List<Minimap.PinData>` type are confirmed accessible — `CollectShareablePins`/`ReadPins` read every one of them today and build 0W/0E.

#### 2N.4 K-B — additive loop in `RebuildOverlay`
...
That is the WHOLE render change. The existing `:713` loop bound-clips (`BoundedMapMath.InDisc`, AT-MAP-BOUND parity), projects (mode-aware `WorldToSurfacePx`/`WorldToSurfacePxSnapped`), and `SpawnPinMarker`s every entry in `rendered` — including the new live bosses — with NO further edits. `SpawnPinMarker` → `ResolvePinSprite` gives the §2K.7 boss icon; the `ShowPrompts` branch gives the §2K.2 `Localize`d label ("Eikthyr", modal only — disc stays glanceable, `ShowPrompts=false`); `AddIfNew` gave K1 dedup; the pin never entering `survey.Pins` gives K3 non-deletability.

#### 2N.5 K1 — dedup (no double-render)

`AddIfNew` (`MapSurface.cs:1290`) already dedups by `(Type, pos within 1 m, name)` against everything already in `rendered` — which includes the frozen `survey.Pins`. A boss in BOTH the frozen survey and the live `m_pins` matches (same `m_type`, same `m_pos`, same `m_name` token, since both read the same vanilla pin) → the live copy is dropped → renders ONCE as the frozen `SurveyPin`. A boss only-live (the bug case) is added. A boss only-frozen (imprinter's, holder hasn't discovered it) stays. No double-render in any case. Zero new dedup code.

#### 2N.6 K2 — DESIGN LOCK: live-overlay for the holder + keep the frozen capture for sharing
...
**Justification (not a hand-wave — grounded in shipped precedent):** ALL FOUR existing live layers — WorldPins MarkerSigns (`:702`), ThreatMarkers (`:743`), and LocationPins (`:786`) — already overlay the *holder's* live data on ANY surface, regardless of who imprinted it. Making boss pins live-overlay too is the **consistent extension** of behavior Daniel has already accepted, not a new semantic. Forcing boss pins to be holder-suppressed on a handed disc would be a special case contradicting every other live layer. The deliberate shared-disc freeze ("a handed disc carries the imprinter's boss discoveries") is **preserved** — the frozen blob is untouched; the live layer is purely additive on top.

> **Daniel override hook (the one alternative, his to veto at the in-game gate):** if you actually want a handed disc to show ONLY the imprinter's frozen bosses and SUPPRESS the holder's own live bosses, say so — but note that contradicts the marker-sign/threat/location layers, which already show the holder's live data on any disc. Default LOCKED to live-overlay (consistent). Boss pins are NOT taken "fully live like location pins" (which would drop the frozen capture and change shared-disc semantics — explicitly NOT wanted).

#### 2N.7 K3 — non-deletable (no §2K.8 regression)

Live boss pins are non-deletable **by construction, two ways:**

1. The TableEdit eraser is a **world-coordinate proximity delete against the PERSISTED `survey.Pins`** (`MapSurface.cs:1584-1585` → `SurveyorTableTag.RemovePinNear` → `ReadSharedSurvey` → `SurveyData.RemovePinNear`, `SurveyData.cs:236`). It is **NOT** a screen raycast on pin GameObjects. A live boss pin is synthesized into the transient `rendered` list and **never enters `survey.Pins`** (same as the live MarkerSign / threat / location pins today), so the eraser literally has nothing to remove.
2. Where a boss IS frozen in `survey.Pins`, the §2K.8 `IsSystemPin` skip in `RemovePinNear` (`SurveyData.cs:242`) already protects it — UNREGRESSED by this change (we only promote the helper's access modifier; the guard logic is untouched).

Live boss pins are **visually identical** to frozen ones (same `SpawnPinMarker` → same §2K.7 icon, same §2K.2 label on the modal, same `PinIconPx`), so a surveyed boss and an unsurveyed boss are indistinguishable on screen.
...
- **No WireVersion bump** — live-derived, persists nothing; `SurveyData.WireVersion` stays `1`.
- **SpecCheck impact: none** — no recipe / piece / station / item change.

#### 2N.9 Files touched + clean/dirty

- **NEW** `src/SBPR.Trailborne/Features/Cartography/SystemPins.cs` — the live boss/Hildir collector (§2N.3).
- `src/SBPR.Trailborne/Features/Cartography/MapSurface.cs` — one `_systemPinScratch` buffer + one additive `SystemPins.Collect` + `AddIfNew` block in `RebuildOverlay` (§2N.4). No change to the bound-clip/project/`SpawnPinMarker` loop, `ResolvePinSprite`, `CounterRotatePins`, or `ClearPinObjects`.
- `src/SBPR.Trailborne/Features/Cartography/SurveyData.cs` — promote `IsSystemPin` `private static` → `internal static` (one access modifier; no logic change) so the live-collect filter reuses the single discriminator (drift-proof with the §2K.8 delete-guard).
- `docs/v2/planning/cartography-impl-spec.md` — this §2N (spec + code in ONE PR, AGENTS.md co-movement).

#### 2N.10 Acceptance tests (named, observable — close only on Daniel's in-game GPU check)

- **AT-VPIN-LIVE-1** — after discovering a boss (e.g. Eikthyr), its pin appears on BOTH SBPR surfaces (Surveyor's-Table modal + carry-disc) WITHOUT requiring a Table survey. Icon = the vanilla boss glyph (§2K.7); modal label = "Eikthyr" (§2K.2); disc = icon-only.
- **AT-VPIN-LIVE-2** — re-activating the boss stone is neither required nor relied upon: the live path surfaces the pin on its own; the boss-stone "already pinned" path stays pure-vanilla and untouched.
- **AT-VPIN-LIVE-3 (K1)** — a boss present in BOTH the frozen survey and the live `m_pins` renders exactly ONCE (no double-render where live + frozen overlap).
- **AT-VPIN-LIVE-4 (K2)** — a shared/handed disc still shows the imprinter's frozen boss discoveries (frozen path intact), PLUS the holder's own live bosses, deduped — consistent with the existing marker-sign / threat / location live layers.
- **AT-VPIN-LIVE-5 (K3)** — live boss/Hildir pins are non-deletable (not `SurveyPin`s → not in `survey.Pins` → the world-proximity eraser cannot touch them); the §2K.8 frozen-pin guard is unregressed; live boss pins are visually identical to frozen ones.
- **AT-VPIN-LIVE-6 (regression)** — SBPR MarkerSign pins, Group-1 frozen boss/Hildir pins (§2K), Sunstone threat blips, and Group-2 location pins (§2M) all still render (this only ADDED a layer); `SurveyData.WireVersion` grep-asserts `1`; `dotnet build … -c Release` = 0 warnings / 0 errors (`TreatWarningsAsErrors` ON).
- **logs-green ≠ playable** — the visual fix and final accept are Daniel's eyeball on a GPU client; a headless box collapses shaders and cannot confirm them.

#### 2N.11 Routing

- **Clean-side → `engineer-ui`** (owns `MapSurface.cs` + the cartography viewer cluster, same as §2K and §2M). Small additive change: 1 new file (~55 lines), 1 promoted access modifier, 1 scratch field, 1 ~8-line `RebuildOverlay` block. **Spec + code in ONE PR** (no standalone docs PR — AGENTS.md spec/code co-movement): paste this §2N into `docs/v2/planning/cartography-impl-spec.md` immediately AFTER §2M (before `## 3. Cartographer's Kit`) in the SAME PR as the code.
- **Scope = Group-1 (Boss + Hildir1–3) ONLY.** Location/merchant pins are §2M (already live). Re-architecting shared-disc freeze semantics beyond the K2 lock is out of scope.
